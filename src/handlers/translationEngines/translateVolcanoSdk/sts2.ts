...
function ZeroPadding(data: Uint8Array, blockSize: number) {
...
function AesEncryptCBCWithBase64(data: any, key: any) {
...
function hmac(type: string, str: string, secret: BinaryLike | KeyObject): string {
  return crypto.createHmac(type, secret).update(str).digest("hex");
}

function hash(type: any, str: string) {
